	...
	String protocol=null;
      ...
      public void run()
      {
		...
		if(p.toString().contains("ICMP")) protocol="icmp";   
			   ...
			   ip=(IPPacket)p;
			   String src="";
			   byte sb[]=tcp.src_ip.getAddress() ;
			   for(int i=1;i<sb.length;i++){
			      int temp=new Byte(sb[sb.length-i]).intValue();
				  src+=temp;
				  }
              if(serviceHash.get(tcp.dst_port)!=null&&kddServ.contains(serviceHash.get(tcp.dst_port).toString()))  System.out.println("Src: " + src+ " Port: " + tcp.src_port + " Dst: " + tcp.dst_ip.getAddress() + "Port: " + tcp.dst_port+"  service is :"+serviceHash.get(tcp.dst_port)+" Protocol is :"+protocol);
